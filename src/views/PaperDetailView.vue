  ...
  <section class="container detail-page">
    <div class="detail-hero card">
      <div>
        ...
        <h2>{{ exam.name }}</h2>
        <p class="detail-copy">
          {{ year }} 年 · {{ exam.duration }} · {{ exam.description }}
        </p>
      </div>
    ...
    <div class="detail-grid">
      <article class="card summary-card">
        ...
        <p v-else class="empty-copy">
          当前这套资料还没有放到 `public/pdf`，可以补齐后自动沿用这一页的展示逻辑。
        </p>
      </article>
      ...
      <article class="card preview-card">
        ...
        <div v-else class="preview-empty">
          ...
          <p>如果你刚补完资料，确认文件名符合 `{年份}_{科目}_题目.pdf`、`答案.pdf` 或 `题目和答案.pdf` 规则即可。</p>
        </div>
      </article>
    </div>
  </section>
...
import { examYears, getExamById, getPdfAssets } from '../data/exams'
...
const currentPreviewTitle = computed(() => {
  if (currentPreview.value === 'answer') {
    return `${year.value} 年答案解析`
  }
