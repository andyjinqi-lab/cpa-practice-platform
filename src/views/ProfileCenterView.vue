  ...
  <section class="container profile-page">
    <div class="profile-grid">
      ...
      <div class="card stats-card">
        ...
        <div class="progress-bar"><i :style="{ width: `${Math.min(100, pendingReviewCount)}%` }"></i></div>
      </div>
    </div>
    ...
    <div v-if="currentReview" ref="reviewCardRef" class="card review-card">
      <div class="review-head">
        <div>
          ...
          <p class="review-copy">可筛选未作答、错题、客观题、主观题。</p>
        </div>
      ...
      <div class="review-list">
        <article v-for="question in filteredQuestions" :key="question.id" class="review-item">
          ...
          <div v-else class="review-answer-block">
            ...
            <p><span>参考解析：</span>{{ question.analysis || '当前暂无结构化答案，请结合 PDF 解析复盘。' }}</p>
          </div>
    ...
    <div v-if="reviewHistory.length" class="card history-card">
      ...
      <div class="history-list">
        <article v-for="record in reviewHistory" :key="record.submittedAt" class="history-item">
          ...
          <button class="wrong-link" @click="openWrongQuestions(record)">错题 {{ getWrongCount(record) }} 题</button>
        </article>
      </div>
    </div>
  </section>
...
import { fetchPracticeReviewData, getSessionEmail } from '../services/auth'
...
onMounted(async () => {
  const data = await fetchPracticeReviewData()
  reviewHistory.value = Array.isArray(data.history)
    ? [...data.history].sort((a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime())
    : []
  latestReview.value = data.latest || reviewHistory.value[0] || null
  selectedSubmittedAt.value = latestReview.value?.submittedAt || reviewHistory.value[0]?.submittedAt || ''
})

const currentReview = computed(() => {
  if (!reviewHistory.value.length) return null
  return reviewHistory.value.find((item) => item.submittedAt === selectedSubmittedAt.value) || reviewHistory.value[0]
})
...
const filteredQuestions = computed(() => {
  ...
  if (activeFilter.value === 'wrong') {
    return questions.filter(
      (question) => question.options?.length && question.answer && hasObjectiveAttempt(question) && !question.isCorrect
    )
  }
  if (activeFilter.value === 'objective') return questions.filter((question) => question.options?.length)
  if (activeFilter.value === 'subjective') return questions.filter((question) => !question.options?.length)
  return questions
})
...
function reviewTagClass(question) {
  if (!question.options?.length) return question.subjectiveAnswer?.trim() ? 'written' : 'empty'
...
function reviewTagLabel(question) {
  if (!question.options?.length) return question.subjectiveAnswer?.trim() ? '已作答' : '未作答'
  if (!question.selectedOptions?.length) return '未作答'
...
.profile-grid { display: grid; grid-template-columns: minmax(0,1.3fr) minmax(280px,0.7fr); gap: 1.25rem; margin-bottom: 1.5rem; }
.eyebrow { color: #d96c2f; font-weight: 800; margin-bottom: 0.5rem; }
.user-meta,.review-summary { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 1rem; margin-top: 1.5rem; }
.user-meta div,.review-summary div { padding: 1rem; border-radius: 18px; background: #f7f9fc; }
.user-meta strong,.review-summary strong { display: block; color: #08203a; font-size: 1.2rem; }
.user-meta span,.review-summary span,.review-copy,.analysis-copy { color: #69758a; }
.progress-item,.review-head,.review-question-head,.history-item { display: flex; justify-content: space-between; gap: 1rem; }
.progress-item { margin: 1rem 0 0.5rem; }
.progress-bar { height: 12px; border-radius: 999px; background: #e9eff7; overflow: hidden; }
.progress-bar i { display: block; height: 100%; border-radius: inherit; background: linear-gradient(135deg,#ffb347,#ff7b54); }
.review-card { margin-bottom: 1.5rem; }
.review-actions,.filter-row { display: flex; gap: 0.75rem; flex-wrap: wrap; }
.review-summary { margin-bottom: 1rem; }
.filter-row { margin-bottom: 1rem; }
.switch-hint { margin-bottom: 1rem; color: #355d9a; background: #edf4ff; border: 1px solid #d7e3f1; border-radius: 12px; padding: 0.55rem 0.75rem; }
.filter-btn { border: 1px solid #d7e3f1; background: #fff; color: #0d5cab; border-radius: 999px; padding: 0.55rem 0.9rem; cursor: pointer; font-weight: 700; }
.filter-btn.active { background: #0d5cab; color: #fff; border-color: #0d5cab; }
.review-list,.history-list { display: grid; gap: 1rem; }
.review-item,.history-item { padding: 1rem; border-radius: 20px; border: 1px solid #e7ecf3; background: #fff; }
.review-question-head p { margin-top: 0.55rem; color: #42556d; line-height: 1.7; }
.review-answer-block { margin-top: 0.9rem; display: grid; gap: 0.45rem; }
.review-answer-block p { color: #31465f; line-height: 1.7; }
.review-answer-block span { color: #69758a; font-weight: 700; margin-right: 0.4rem; }
.analysis-copy { margin-top: 0.9rem; line-height: 1.8; }
.wrong-link { border: 1px solid #d7e3f1; background: #edf4ff; color: #0d5cab; border-radius: 999px; padding: 0.45rem 0.85rem; font-weight: 700; cursor: pointer; }
.result-tag { display: inline-flex; align-items: center; height: fit-content; padding: 0.4rem 0.75rem; border-radius: 999px; font-weight: 700; }
.result-tag.correct { background: #eff8f1; color: #2b7b4d; }
.result-tag.wrong { background: #fff0eb; color: #c75d38; }
.result-tag.pending,.result-tag.written { background: #edf4ff; color: #355d9a; }
.result-tag.empty { background: #f4f6fa; color: #8b96a8; }
@media (max-width: 960px) {
  .profile-grid { grid-template-columns: 1fr; }
  .user-meta,.review-summary { grid-template-columns: 1fr; }
