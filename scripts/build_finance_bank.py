...
TEXT_ROOT = ROOT / "generated" / "question_bank_text" / "finance"
OUTPUT_ROOT = ROOT / "generated" / "structured_question_bank" / "finance"
PUBLIC_OUTPUT = ROOT / "public" / "data" / "finance-question-bank.json"
...
def parse_answers(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    ...
    while index < len(lines):
        ...
        if match and current_section_name:
            ...
            # OCR may drop the "多项选择题" heading; detect numeric reset and switch section.
            if (
                current_section_name == "单项选择题"
                and number == 1
                and len(data.get("单项选择题", {})) >= 10
                and not data.get("多项选择题")
            ):
...
def parse_2025_answer_blocks(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    section_titles = ["一、单项选择题", "二、多项选择题", "三、计算分析题"]
    section_name_map = {
        "一、单项选择题": "单项选择题",
        "二、多项选择题": "多项选择题",
        "三、计算分析题": "计算分析题",
    ...
    for raw_line in answer_text.splitlines():
        ...
        if (
            ...
            or line.startswith("《真题回忆版》财务成本管理")
...
def parse_finance_2025_combined(combined_text: str, year: int, source_name: str) -> list[QuestionRecord]:
    body = clean_text(body_only(combined_text))
    starts = [match.start() for match in re.finditer("一、单项选择题", body)]
    if len(starts) < 2:
        combined_records = merge_combined_text(combined_text, year, source_name)
        return [clean_question_record(item) for item in normalize_section_numbers(combined_records)]
    ...
    normalized = normalize_section_numbers(questions)
    return [clean_question_record(item) for item in normalized]
...
def normalize_section_numbers(records: list[QuestionRecord]) -> list[QuestionRecord]:
    counters: dict[str, int] = {}
    for question in records:
        counters.setdefault(question.section_type, 0)
        counters[question.section_type] += 1
        question.number = counters[question.section_type]
    return records


def clean_stem_text(stem: str) -> str:
    cleaned = stem.strip()
    cleaned = re.sub(r"\d{4}年度注会《[^》]+》试题", "", cleaned)
...
def clean_question_record(question: QuestionRecord) -> QuestionRecord:
    ...
    apply_finance_overrides(question)
    return question


def apply_finance_overrides(question: QuestionRecord) -> None:
    key = (question.year, question.section_type, question.number)

    answer_overrides: dict[tuple[int, str, int], str] = {
        (2014, "single", 13): "D",
        (2014, "single", 14): "C",
        (2017, "single", 14): "D",
        (2018, "single", 14): "C",
    ...
    if not question.answer and key in answer_overrides:
        question.answer = answer_overrides[key]

    option_overrides: dict[tuple[int, str, int], dict[str, str]] = {
        (2020, "single", 4): {
            "A": "980元",
            "B": "990元",
            "C": "1000元",
    ...
    if key in option_overrides and len(question.options) < 4:
        question.options = option_overrides[key]

    stem_overrides: dict[tuple[int, str, int], str] = {
        (2024, "single", 16): (
            "甲公司拟发行非上市交易的10年期公司债券，信用等级为A。"
            "目前市场上有剩余期限10年的国债，到期收益率3.6%，"
            "A 级债券信用风险溢价1.3%，非上市交易债券流动风险溢价1.7%。"
            "债券平价发行，票面利率为( )。"
        ),
        (2024, "multiple", 6): (
            "公司拟发行可转换债券，单利计息、到期一次还本付息。"
            "在其他条件不变的情况下，下列会使资本成本提高的有( )。"
        ),
    }

    if key in stem_overrides:
        if key == (2024, "multiple", 6):
            question.stem = stem_overrides[key]
        elif not question.stem or len(question.stem) < 20:
            question.stem = stem_overrides[key]

    # Fallback fixes that survive renumbering: match by stem features.
    if (
        question.year == 2020
        and question.section_type == "single"
        and "经济增加值" in question.stem
        and "净利润10亿元" in question.stem
    ...
    if (
        question.year == 2024
        and question.section_type == "multiple"
        and "转换价值" in question.stem
        and "转换比率" in question.stem
    ):
        question.stem = (
            "公司拟发行可转换债券，单利计息、到期一次还本付息。"
            "在其他条件不变的情况下，下列会使资本成本提高的有( )。"
...
def build_year_bank(year: int) -> list[QuestionRecord]:
    ...
    if combined_file:
        if year == 2025:
            return parse_finance_2025_combined(
                combined_file.read_text(encoding="utf-8"),
                year,
                combined_file.name,
            )
        combined_records = merge_combined_text(combined_file.read_text(encoding="utf-8"), year, combined_file.name)
        normalized_records = normalize_section_numbers(combined_records)
        return [clean_question_record(item) for item in normalized_records]
    ...
    collapsed_records = collapse_subjective_subquestions(questions)
    normalized_records = normalize_section_numbers(collapsed_records)
    return [clean_question_record(item) for item in normalized_records]
...
def to_practice_item(question: QuestionRecord) -> dict[str, object]:
    ...
    fingerprint = hashlib.md5(question.stem.encode("utf-8")).hexdigest()[:8]
    return {
        "id": f"finance-{question.year}-{question.section_type}-{question.number}-{fingerprint}",
        "year": question.year,
        "subject": "财务成本管理",
        ...
        "title": f"{question.year}年财务成本管理{question.section_name}第{question.number}题",
        "stem": question.stem,
