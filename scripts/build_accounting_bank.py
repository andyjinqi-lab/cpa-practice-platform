...
TEXT_ROOT = ROOT / "generated" / "question_bank_text" / "accounting"
OUTPUT_ROOT = ROOT / "generated" / "structured_question_bank" / "accounting"
PUBLIC_OUTPUT = ROOT / "public" / "data" / "accounting-question-bank.json"
...
def parse_answers(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    ...
    while index < len(lines):
        ...
        if current_section_name in {"计算分析题", "综合题"}:
            major_question_match = MAJOR_QUESTION_PATTERN.match(line)
            if major_question_match:
...
def parse_2025_answer_blocks(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    section_titles = ["一、单项选择题", "二、多项选择题", "三、计算分析题", "四、综合题"]
    section_name_map = {
        "一、单项选择题": "单项选择题",
        "二、多项选择题": "多项选择题",
        "三、计算分析题": "计算分析题",
...
def parse_2025_question_blocks(question_text: str, year: int, source_name: str) -> list[QuestionRecord]:
    ...
    for raw_line in question_text.splitlines():
        ...
        if current_section_type in {"single", "multiple"}:
            ...
            # OCR fallback: some numbers (e.g., "2.") are lost. If previous question already has
            # full options and a new sentence starts with a stem-like ending, start an implicit item.
            if (
                current_question
        ...
        # subjective
        subjective_match = re.match(r"^(\d+)[\.\、．]\s*(.*)$", line)
        if subjective_match:
            question_number = int(subjective_match.group(1))
            remainder = subjective_match.group(2).strip()
            if question_number <= 0 or len(remainder) < 6:
                continue
            question = create_question(question_number, remainder)
            records.append(question)
            current_question = question
            current_option_key = None
            continue
...
def parse_accounting_2025_combined(combined_text: str, year: int, source_name: str) -> list[QuestionRecord]:
...
def collapse_subjective_subquestions(records: list[QuestionRecord]) -> list[QuestionRecord]:
    ...
    for question in records:
        ...
        if looks_like_major_subjective_prompt(question):
            collapsed.append(question)
            continue
...
def build_year_bank(year: int) -> list[QuestionRecord]:
    ...
    if combined_file:
        if year == 2025:
            return parse_accounting_2025_combined(
...
def to_practice_item(question: QuestionRecord) -> dict[str, object]:
    ...
    return {
        "id": f"accounting-{question.year}-{question.section_type}-{question.number}",
        "year": question.year,
        "subject": "会计",
        ...
        "title": f"{question.year}年会计{question.section_name}第{question.number}题",
        "stem": question.stem,
