...
def parse_2025_answer_blocks(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    section_titles = ["一、单项选择题", "二、多项选择题", "三、计算问答题", "四、综合题"]
    section_name_map = {
        "一、单项选择题": "单项选择题",
        "二、多项选择题": "多项选择题",
        "三、计算问答题": "计算问答题",
...
def parse_tax_2025_combined(combined_text: str, year: int, source_name: str) -> list[QuestionRecord]:
...
def clean_question_record(question: QuestionRecord) -> QuestionRecord:
    ...
    apply_tax_overrides(question)
    return question


def apply_tax_overrides(question: QuestionRecord) -> None:
    # Disabled by default for tax; add year-specific fixes only when verified.
    return None

def _legacy_finance_overrides_only_for_reference(question: QuestionRecord) -> None:
...
def build_year_bank(year: int) -> list[QuestionRecord]:
    ...
    if combined_file:
        if year == 2025:
            return parse_tax_2025_combined(
...
def to_practice_item(question: QuestionRecord) -> dict[str, object]:
    ...
    return {
        "id": f"tax-{question.year}-{question.section_type}-{question.number}-{fingerprint}",
        "year": question.year,
        "subject": "税法",
        ...
        "title": f"{question.year}年税法{question.section_name}第{question.number}题",
        "stem": question.stem,
