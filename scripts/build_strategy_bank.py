...
TEXT_ROOT = ROOT / "generated" / "question_bank_text" / "strategy"
OUTPUT_ROOT = ROOT / "generated" / "structured_question_bank" / "strategy"
PUBLIC_OUTPUT = ROOT / "public" / "data" / "strategy-question-bank.json"
...
def parse_answers(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    ...
    while index < len(lines):
        ...
        if line.startswith("【答案】") and current_number is not None:
            inline = re.sub(r"\s+", "", line.replace("【答案】", ""))
            if re.fullmatch(r"[A-D]+", inline):
                data.setdefault(current_answer_section_name, {}).setdefault(current_number, {})["answer"] = inline
                index += 1
                continue
            ...
            while look_ahead < len(lines):
                ...
                if re.fullmatch(r"[A-D]+", compact_candidate):
                    data.setdefault(current_answer_section_name, {}).setdefault(current_number, {})["answer"] = compact_candidate
                    index = look_ahead
                break
            index += 1
            continue
        ...
        if match and current_section_name:
            ...
            answer_tokens = ["【答案】", "[答案]", "【参考答案】", "【正确答案】", "答案：", "答案:"]
            ...
            if not direct_answer and (not rest or any(token in rest for token in answer_tokens)):
                saw_answer_marker = any(token in rest for token in answer_tokens)
                fallback_letter = ""
                look_ahead = index + 1
                while look_ahead < len(lines):
                    candidate = lines[look_ahead].strip()
                    ...
                    if normalize_section_title(candidate) or QUESTION_START_PATTERN.match(candidate):
                        break
                    inline_candidate_answer = extract_inline_answer(candidate)
                    if inline_candidate_answer:
                        direct_answer = inline_candidate_answer
                        index = look_ahead
                        break
                    compact_candidate = re.sub(r"\s+", "", candidate)
                    if any(token in candidate for token in answer_tokens):
                        saw_answer_marker = True
                    if re.fullmatch(r"[A-D]+", compact_candidate):
                        fallback_letter = compact_candidate
                        if saw_answer_marker:
                            direct_answer = compact_candidate
                            index = look_ahead
                            break
                    look_ahead += 1
                if not direct_answer and fallback_letter and (saw_answer_marker or not rest):
                    direct_answer = fallback_letter
...
def parse_2025_answer_blocks(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    section_titles = ["一、单项选择题", "二、多项选择题", "三、简答题", "四、综合题"]
    section_name_map = {
        "一、单项选择题": "单项选择题",
        "二、多项选择题": "多项选择题",
        "三、简答题": "简答题",
...
def parse_strategy_2025_combined(combined_text: str, year: int, source_name: str) -> list[QuestionRecord]:
...
def apply_audit_overrides(question: QuestionRecord) -> None:
    # No subject-specific overrides for strategy.
    return None
...
def build_year_bank(year: int) -> list[QuestionRecord]:
    ...
    if combined_file:
        if year == 2025:
            return parse_strategy_2025_combined(
    ...
    for question in questions:
        ...
        if not answer_bucket:
            fallback_section_name = get_section_title_by_type(question.section_type)
            answer_bucket = answer_map.get(fallback_section_name, {}).get(question.number, {})
...
def to_practice_item(question: QuestionRecord) -> dict[str, object]:
    ...
    return {
        "id": f"strategy-{question.year}-{question.section_type}-{question.number}",
        "year": question.year,
        "subject": "公司战略与风险管理",
        ...
        "title": f"{question.year}年战略{question.section_name}第{question.number}题",
        "stem": question.stem,
