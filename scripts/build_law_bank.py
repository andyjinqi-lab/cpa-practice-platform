...
TEXT_ROOT = ROOT / "generated" / "question_bank_text" / "law"
OUTPUT_ROOT = ROOT / "generated" / "structured_question_bank" / "law"
PUBLIC_OUTPUT = ROOT / "public" / "data" / "law-question-bank.json"
...
def parse_2025_answer_blocks(answer_text: str) -> dict[str, dict[int, dict[str, str]]]:
    section_titles = ["一、单项选择题", "二、多项选择题", "三、案例分析题"]
    section_name_map = {
        "一、单项选择题": "单项选择题",
        "二、多项选择题": "多项选择题",
        "三、案例分析题": "案例分析题",
...
def parse_law_2025_combined(combined_text: str, year: int, source_name: str) -> list[QuestionRecord]:
...
def apply_audit_overrides(question: QuestionRecord) -> None:
    # No subject-specific overrides for law.
    return None
...
def build_year_bank(year: int) -> list[QuestionRecord]:
    ...
    if combined_file:
        if year == 2025:
            return parse_law_2025_combined(
...
def to_practice_item(question: QuestionRecord) -> dict[str, object]:
    ...
    return {
        "id": f"law-{question.year}-{question.section_type}-{question.number}",
        "year": question.year,
        "subject": "经济法",
        ...
        "title": f"{question.year}年经济法{question.section_name}第{question.number}题",
        "stem": question.stem,
